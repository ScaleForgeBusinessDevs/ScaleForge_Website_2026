...
export const projectType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: "array",
      description: "Key outcomes — e.g. 312% traffic increase, 4.2s → 0.8s load time",
      of: [
        {
          type: "object",
          fields: [
            defineField({ name: "value", title: "Value (e.g. 312%)", type: "string", validation: (r) => r.required() }),
            defineField({ name: "label", title: "Label (e.g. Traffic increase)", type: "string", validation: (r) => r.required() }),
          ],
          preview: {
    ...
    defineField({
      ...
      type: "text",
      rows: 4,
      description: "What problem did the client face before engaging ScaleForge?",
    }),
    defineField({
      name: "solution",
      title: "Our Solution",
  ...
  preview: {
    select: { title: "title", media: "coverImage", category: "category", client: "client" },
