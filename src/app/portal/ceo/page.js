...
export default function CeoPortal() {
  ...
  const [rates, setRates] = useState({ USD: 278.5, EUR: 300.2, GBP: 350 });
  ...
  const loadRates = useCallback(async () => {
    ...
    if (data && data.length > 0) {
      const rMap = { USD: 278.5, EUR: 300.2, GBP: 350 };
      data.forEach((r) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (prof.role !== "ceo") {
        router.replace(
          prof.role === "admin" ? "/portal/admin" : "/portal/employee",
        );
        return;
      }
      setProfile(prof);
                ...
                {group.items.map((tab) => (
                  ...
                  >
                    <tab.icon
                      size={18}
                      className={
                        activeTab === tab.id ? "text-green-400" : "opacity-70"
                      }
                    />
                    {tab.label}
                  </button>
        ...
        <div className="p-8 max-w-[1400px] w-full mx-auto">
          ...
          >
            ...
            {activeTab === "job-openings" && (
              <CeoJobOpeningsTab addToast={addToast} />
            )}
            {activeTab === "analytics" && (
              <CeoAnalyticsTab
                addToast={addToast}
            ...
            {activeTab === "settings" && (
              <CeoSettingsTab addToast={addToast} onRatesSaved={loadRates} />
            )}
            {activeTab === "onboarding" && (
              <OnboardingTab addToast={addToast} />
            )}
          </motion.div>
        </div>
      </div>
