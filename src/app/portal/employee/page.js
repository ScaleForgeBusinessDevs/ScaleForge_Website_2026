...
const ATTENDANCE_SEQUENCE = ["time_in", "break_start", "break_end", "time_out"];
...
const BTN_META = {
  ...
  time_out: {
    label: "Time Out",
    icon: LogOut,
    style: "bg-red-500/10 border-red-500/30 text-red-400 hover:bg-red-500/20",
  },
};
...
export default function EmployeePortal() {
  ...
  const refreshAttendanceState = useCallback(async (profileId) => {
    ...
    <main className="min-h-screen bg-black text-white selection:bg-orange-500/20">
      ...
      <div className="max-w-[1400px] mx-auto px-6 pt-28 pb-16 space-y-6">
        ...
        >
          ...
          {activeTab === "tasks" && (
            <ActiveTasksTab profileId={profile.id} addToast={addToast} />
          )}
          {activeTab === "attendance" && (
            <AttendanceHistoryTab profileId={profile.id} />
          )}
          {activeTab === "calendar" && (
            <CalendarTab role="employee" addToast={addToast} />
          )}
