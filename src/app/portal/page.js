    ...
    (async () => {
      ...
      if (session) {
        ...
        if (profile) {
          ...
          setError(
            "Account setup incomplete: No profile found. Please contact an admin.",
          );
          setChecking(false);
        }
      } else {
  ...
  return (
    <main className="min-h-screen w-full flex bg-white text-black overflow-hidden font-sans">
      ...
      <div className="relative hidden lg:flex w-1/2 bg-gray-100 flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="relative z-10 text-white pb-8">
          ...
          <p className="text-white/80 text-sm max-w-sm">
            Streamline operations, track analytics, and optimize your workflows
      ...
      <div className="w-full lg:w-1/2 flex flex-col items-center justify-center p-8 lg:p-24 relative bg-white">
        ...
        >
          <h1 className="text-3xl font-bold text-[#0A0A0B] tracking-tight mb-2">
            Welcome Back to ScaleForge!
          </h1>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <div className="flex items-center justify-between mt-2">
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  type="checkbox"
                  className="w-4 h-4 rounded border-[#D1D5DB] text-black focus:ring-black accent-black cursor-pointer"
                />
                <span className="text-sm text-[#4B5563]">Remember Me</span>
              </label>
              <button
                type="button"
                className="text-sm text-[#9CA3AF] hover:text-[#374151] hover:underline transition-colors"
              >
                Forgot Password?
              </button>
            </div>
