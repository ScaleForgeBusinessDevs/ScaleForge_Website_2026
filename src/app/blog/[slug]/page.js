...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const imgUrl = post.featuredImage?.asset
      ...
      : undefined;
    let finalTitle = post.seoTitle;
    if (!finalTitle) {
      const suffix = " | ScaleForge";
      const maxTitleLen = 70 - suffix.length;
      if (post.title.length > maxTitleLen) {
        finalTitle = post.title.substring(0, maxTitleLen - 3) + "..." + suffix;
      } else {
    ...
    return {
      ...
      alternates: {
    canonical: `https://scalesforge.site/blog/${slug}`,
    languages: {
      "en": `https://scalesforge.site/blog/${slug}`,
      "x-default": `https://scalesforge.site/blog/${slug}`
    },
    media: {
      "only screen and (max-width: 640px)": `https://scalesforge.site/blog/${slug}`
    }
  },
      openGraph: {
        title: finalTitle,
        description: post.excerpt,
        url: `https://scalesforge.site/blog/${slug}`,
        siteName: "ScaleForge",
        images: imgUrl
          ? [{ url: imgUrl, width: 1200, height: 630 }]
          : undefined,
        type: "article",
...
const ptComponents = {
  block: {
    h1: ({ children }) => (
      <h2 className="mb-4 mt-10 text-[26px] font-medium leading-snug text-white sm:text-[30px]">
        {children}
      </h2>
    ),
    h2: ({ children }) => (
      <h2 className="mb-4 mt-10 text-[22px] font-medium leading-snug text-white sm:text-[25px]">
        {children}
      </h2>
    ),
    h3: ({ children }) => (
      <h3 className="mb-3 mt-8 text-[19px] font-medium leading-snug text-white">
        {children}
      </h3>
    ),
    normal: ({ children }) => (
      <p className="mb-5 text-[16.5px] leading-[1.7] text-white/60 sm:text-[17.5px]">
        {children}
      </p>
    ),
    blockquote: ({ children }) => (
  ...
  list: {
    bullet: ({ children }) => (
      <ul className="mb-5 ml-1 space-y-2.5 text-[16px] leading-relaxed text-white/55">
        {children}
      </ul>
    ),
    number: ({ children }) => (
      <ol className="mb-5 ml-4 list-decimal space-y-2.5 text-[16px] leading-relaxed text-white/55">
        {children}
      </ol>
    ),
  },
  ...
  marks: {
    ...
    link: ({ children, value }) => {
      let href = value?.href || "";
      href = href.replace("scaleforgewebdev.vercel.app", "scalesforge.site");
      
      let hasText = false;
      if (typeof children === 'string' && children.trim().length > 0) {
        hasText = true;
      } else if (Array.isArray(children)) {
        hasText = children.some(c => typeof c === 'string' && c.trim().length > 0);
      }
      const content = hasText ? children : "ScaleForge Home";
    ...
    image: ({ value }) => {
      ...
      return (
        <figure className="my-8">
          ...
          />
          {value.caption && (
            <figcaption className="mt-2 text-center text-[12.5px] italic text-white/35">
              {value.caption}
            </figcaption>
          )}
        </figure>
...
function getTopicServiceLink(topicSlug) {
  const map = {
  ...
  return (
    map[topicSlug ?? ""] ?? {
      href: "/services",
      label: "high-performance digital systems",
    }
  );
}
...
export default async function BlogPostPage({ params }) {
  ...
  const blogPostSchema = post ? {
    ...
    "@type": "BlogPosting",
    "headline": post.title,
    "image": post.featuredImage?.asset ? urlFor(post.featuredImage).url() : undefined,
    "datePublished": post.publishedDate,
    "description": post.excerpt,
    "mainEntityOfPage": {
      "@type": "WebPage",
      "@id": `https://scalesforge.site/blog/${slug}`
    },
    "author": {
      "@type": "Person",
    ...
    "publisher": {
      ...
      "logo": {
        "@type": "ImageObject",
        "url": "https://scalesforge.site/Assets/favicon_SF.png"
      }
    }
  } : null;
          ...
          <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
            ...
            <Reveal>
              ...
              <div className="flex items-center gap-3">
                {primaryTopic && (
                  <span className="rounded-full border border-white/10 px-3 py-1 font-accent text-[10px] uppercase tracking-[0.12em] text-white/60">
                    {primaryTopic.title}
                  </span>
                )}
              ...
              <div className="mt-5 flex flex-wrap items-center gap-4 text-[13px] text-white/40">
                ...
                {post.lastUpdatedDate &&
                  post.lastUpdatedDate !== post.publishedDate && (
                    <span className="text-[#e8633a]/70">
                      Updated {formatDate(post.lastUpdatedDate)}
                    </span>
                  )}
          ...
          <div className="bg-[#08080a] py-6">
            <div className="mx-auto max-w-[1100px] px-6 lg:px-10">
              <img
                ...
                className="w-full rounded-3xl aspect-[1.905] object-cover bg-white/[0.03]"
              />
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 gap-16 lg:grid-cols-[1fr_280px]">
              ...
              <aside className="hidden lg:block">
                <div className="sticky top-32 flex flex-col gap-5">
                  ...
                  {/* Share */}
                  <div className="rounded-2xl border border-white/[0.07] bg-[#101013] p-5">
                    <p className="font-accent text-[10px] uppercase tracking-[0.14em] text-white/35">
                      Share
                    </p>
                ...
                <GlassBlogCard
                  ...
                  image={
                    p.featuredImage?.asset
                      ? urlFor(p.featuredImage).width(480).height(270).url()
                      : null
                  }
      ...
      <section className="bg-[#08080a] py-16 lg:py-20">
        <div className="mx-auto max-w-[520px] px-6 lg:px-10">
          <Reveal className="rounded-2xl border border-white/[0.07] bg-[#101013] p-8 text-center">
            ...
            <p className="mt-2 text-[13px] leading-relaxed text-white/60">
              One actionable tip per week. No spam. No filler. Unsubscribe with
              one click.
            </p>
