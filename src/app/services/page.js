...
export const metadata = {
  ...
  alternates: {
    canonical: "https://scalesforge.site/services",
    languages: {
  ...
  const servicesSchema = {
    ...
    "itemListElement": SERVICES.map((s, i) => ({
      ...
      "item": {
        ...
        "url": `https://scalesforge.site${s.href}`,
        "provider": {
          "@type": "Organization",
          "name": "ScaleForge"
        }
        ...
        <div className="mx-auto max-w-[860px] px-6 text-center lg:px-10">
          <Reveal>
            ...
            <p className="mx-auto mt-6 max-w-2xl text-[15px] leading-relaxed text-white/60">
              We don&apos;t believe in half-measures. Having a gorgeous website
              means nothing if no one can find it on search. Ranking on page one
              of Google is worthless if your page is slow and fails to convert.
              And running expensive ad campaigns is a waste if your business
              still answers leads manually at 9am the next morning.
            </p>
            ...
            {SERVICES.map((service, i) => (
              <Reveal
                ...
                className={`overflow-hidden rounded-3xl border bg-[#101013] ${
                  i === 0
                    ? "border-white/[0.14] shadow-[0_40px_120px_-50px_rgba(255,255,255,0.2)]"
                    : "border-white/[0.07]"
                }`}
              >
                <div className="grid grid-cols-1 gap-10 p-8 lg:grid-cols-[1.1fr_1fr] lg:gap-16 lg:p-12">
                  <div>
                    ...
                    <h2 className="mt-5 text-[24px] font-medium leading-snug text-white sm:text-[28px]">
                      {service.name}
                    </h2>
                    <h3 className="mt-2 text-[15px] leading-snug text-white/55">
                      {service.valueProp}
                    </h3>
                  ...
                  <div className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-7">
                    ...
                    <ul className="mt-5 space-y-3">
                      {service.included.map((item) => (
                        <li
                          key={item}
                          className="flex items-start gap-2.5 text-[13px] leading-relaxed text-white/55"
                        >
