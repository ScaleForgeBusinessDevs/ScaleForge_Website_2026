...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    let title = "";
    if (project.title.length > maxTitleLen) {
      title = project.title.substring(0, maxTitleLen - 3) + "..." + suffix;
    } else {
...
const ptComponents = {
  block: {
    ...
    normal: ({ children }) => (
      <p className="mb-5 text-[16.5px] leading-[1.7] text-white/60">
        {children}
      </p>
    ),
    blockquote: ({ children }) => (
  ...
  list: {
    ...
    number: ({ children }) => (
      <ol className="mb-5 ml-5 list-decimal space-y-2.5 text-[16px] leading-relaxed text-white/55">
        {children}
      </ol>
    ),
  },
    ...
    link: ({ children, value }) => {
      ...
      return (
        <a
          ...
          className="text-[#93b4ff] underline decoration-[#5e6ad2]/40 underline-offset-4 hover:decoration-[#93b4ff]/60"
        >
          {content}
        </a>
      );
    ...
    image: ({ value }) => {
      ...
      return (
        <figure className="my-8">
          ...
          {value.caption && (
            <figcaption className="mt-3 text-center text-[13px] text-white/35">
              {value.caption}
            </figcaption>
          )}
        </figure>
        ...
        <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
          <Reveal>
            ...
            <nav className="mb-6 flex items-center gap-2 font-accent text-[10px] uppercase tracking-[0.14em] text-white/35">
              ...
              <Link href="/projects" className="hover:text-white/60">
                Projects
              </Link>
              <ChevronRight size={10} className="text-white/35" aria-hidden />
            ...
            <div className="flex flex-wrap items-center gap-3">
              <span
                className={`rounded-full border px-3 py-1 font-accent text-[10px] uppercase tracking-[0.12em] ${categoryStyle}`}
              >
                {project.category}
              </span>
            ...
            <div className="min-w-0">
                  ...
                  <div className="mb-12 grid grid-cols-1 gap-5 sm:grid-cols-2">
                    {project.challenge && (
                      <div className="rounded-2xl border border-white/[0.07] bg-[#101013] p-7">
                        <p className="mb-3 font-accent text-[10px] uppercase tracking-[0.15em] text-white/35">
                          The Challenge
                        </p>
                    ...
                    {project.solution && (
                      <div className="rounded-2xl border border-[#5e6ad2]/20 bg-[#5e6ad2]/[0.05] p-7">
                        <p className="mb-3 font-accent text-[10px] uppercase tracking-[0.15em] text-[#a5aef0]/60">
                          Our Solution
                        </p>
                        <p className="text-[15px] leading-relaxed text-white/65">
                          {project.solution}
              ...
              {project.gallery && project.gallery.length > 0 && (
                <Reveal className="mt-14">
                  <h2 className="mb-6 text-[18px] font-medium text-white">
                    Project Gallery
                  </h2>
                  <ProjectGallery
            ...
            <aside className="space-y-5">
              <Reveal>
                <div className="rounded-2xl border border-white/[0.07] bg-[#101013] p-6">
                  <p className="mb-4 font-accent text-[10px] uppercase tracking-[0.15em] text-white/35">
                    Project Details
                  </p>
                  <dl className="space-y-3.5">
                    <div>
                      <dt className="text-[11px] uppercase tracking-[0.08em] text-white/30">
                        Category
                      </dt>
                      <dd className="mt-0.5 text-[14px] text-white/75">
                        {project.category}
                      </dd>
                    </div>
                    {project.client && (
                ...
                <div className="mt-5 rounded-2xl border border-[#2563eb]/20 bg-[#2563eb]/[0.06] p-6">
                  ...
                  <p className="mt-2 text-[13px] leading-relaxed text-white/60">
                    Let&apos;s discuss what ScaleForge can build for your
                    business.
                  </p>
                  <Link
