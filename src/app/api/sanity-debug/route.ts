...
export async function GET() {
  try {
    ...
    const allDocs = await sanityClient.fetch(`*[!(_type match "system.*") && !(_type match "sanity.*")][0..9]`);

    return NextResponse.json({ projectId, dataset, types, samples, allDocs }, { status: 200 });
  } catch (err: unknown) {
    return NextResponse.json(
      { error: String(err), dataset: process.env.NEXT_PUBLIC_SANITY_DATASET, projectId: process.env.NEXT_PUBLIC_SANITY_PROJECT_ID },
      { status: 500 }
    );
  }
}
