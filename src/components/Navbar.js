...
const LINKS = [
  ...
  { label: "Solutions", href: "/solutions" },
  {
    ...
    children: [
      ...
      { label: "Motion Analysis", href: "/services/motion-analysis" },
    ],
  },
    ...
    <header ref={ref} className="fixed inset-x-0 top-4 z-50 px-4 lg:px-6">
      <nav className="relative mx-auto flex max-w-[1440px] items-center justify-between overflow-visible rounded-2xl border border-white/[0.12] bg-white/[0.05] px-6 py-3.5 shadow-[0_8px_40px_-12px_rgba(0,0,0,0.55)] backdrop-blur-2xl backdrop-saturate-150 lg:px-8">
        ...
        <Link
          ...
          className="relative z-10 flex items-center gap-2 text-[15px] font-semibold tracking-tight"
        >
          <img
          ...
          {LINKS.map((link) =>
            link.children ? (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1.5 transition-colors hover:text-[#a5aef0]"
                >
                  {link.label}
                  <ChevronDown
                    size={12}
              ...
              <li key={link.label}>
                <Link
                  href={link.href}
                  className="transition-colors hover:text-[#a5aef0]"
                >
                  {link.label}
                </Link>
              </li>
