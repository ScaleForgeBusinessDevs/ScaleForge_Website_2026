...
import { LINKEDIN_POSTS, LINKEDIN_COMPANY_URL, LinkedInPost } from "@/config/linkedin-posts";
...
function PostCard({ post }: { post: LinkedInPost }) {
  return (
    <div
      className="group flex flex-col justify-between gap-6 rounded-2xl border border-white/[0.07] bg-[#101013] p-7 transition-all duration-300 hover:border-white/[0.14] hover:bg-[#131316]"
    >
      {/* Header */}
      ...
      <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
        <Reveal className="mx-auto flex max-w-3xl flex-col items-center text-center">
          ...
          <p className="mt-4 text-[15px] leading-relaxed text-white/45">
            Real insights, campaign results, and branding breakdowns — pulled from our LinkedIn.
          </p>
        </Reveal>

        {posts && posts.length > 0 ? (
          <Reveal stagger staggerAmount={0.08} className="mt-14 grid grid-cols-1 gap-5 md:grid-cols-3">
            {posts.map((post) => (
              <PostCard key={post.id} post={post} />
            ))}
          </Reveal>
        ) : (
          <Reveal className="mt-14 text-center text-white/45">
