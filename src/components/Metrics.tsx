  ...
  return (
    <section className="bg-[#08080a] py-24 lg:py-32">
      ...
      <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
        <Reveal stagger staggerAmount={0.12} className="grid grid-cols-1 gap-6 sm:grid-cols-3">
          {METRICS.map((metric) => (
            <div key={metric.index} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-7">
              <div className="flex items-center justify-between">
                <i
                  ...
                  aria-hidden
                />
                <span className="text-[10.5px] uppercase tracking-[0.18em] text-white/35">{metric.index} // Metrics</span>
              </div>

              <p
                className="mt-6 text-[42px] font-bold leading-none tracking-[-0.02em] text-white"
                style={{ textShadow: "0 0 56px rgba(255, 255, 255, 0.55), 0 0 18px rgba(255, 255, 255, 0.45)" }}
              >
                {metric.stat}
              </p>

              <h3 className="mt-4 text-[15px] font-semibold text-white">{metric.title}</h3>
              <p className="mt-2 text-[13.5px] leading-relaxed text-white/40">{metric.body}</p>
            </div>
          ))}
        </Reveal>
      </div>
