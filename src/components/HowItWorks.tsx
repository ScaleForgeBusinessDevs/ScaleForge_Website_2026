  ...
  if (kind === "slider") {
    return (
      <div className="flex w-full flex-col gap-3 px-4">
        <span className="text-[10.5px] text-white/30">Confidence threshold</span>
    ...
    <div className="grid w-full grid-cols-8 gap-1.5 px-6">
      {Array.from({ length: 32 }).map((_, i) => (
        <span
          ...
          style={{ background: `rgba(255,255,255,${0.08 + ((i * 37) % 60) / 220})` }}
        />
      ))}
    </div>
  );
    ...
    <section className="border-y border-white/[0.06] bg-[#0c0c0f] py-24 lg:py-32">
      <div className="mx-auto max-w-[1440px] px-6 lg:px-10">
        ...
        <Reveal stagger className="mt-16 grid grid-cols-1 gap-6 lg:grid-cols-3">
          {STEPS.map((step) => (
            <div key={step.n} className="flex flex-col gap-6 rounded-2xl border border-white/[0.07] bg-[#101013] p-7">
              <div>
                <span className="text-[12px] font-medium text-white/30">{step.n}</span>
                <h3 className="mt-2 text-[16px] font-medium text-white">{step.title}</h3>
                <p className="mt-2 text-[13.5px] leading-relaxed text-white/40">{step.body}</p>
              </div>
