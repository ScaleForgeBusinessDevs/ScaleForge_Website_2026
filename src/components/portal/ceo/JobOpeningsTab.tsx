...
import { Plus, Briefcase, Trash2, CheckCircle2 } from "lucide-react";

interface JobOpeningsTabProps {
  addToast: (type: "success" | "error" | "info", msg: string) => void;
}

export default function JobOpeningsTab({ addToast }: JobOpeningsTabProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast("error", error.message || "Failed to create job. Did you add SANITY_API_TOKEN?");
    } finally {
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            <h2 className="text-xl font-bold text-white tracking-tight">Create Job Opening</h2>
            <p className="text-white/40 text-sm">Publish new roles directly to the careers page.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-1 md:col-span-2 space-y-2">
              <label className="text-xs font-bold text-white/60 uppercase tracking-wider">Requirements (One per line)</label>
              <textarea
                required
                rows={4}
            ...
            <div className="col-span-1 md:col-span-2 flex items-center gap-3">
              ...
              <span className="text-sm text-white/70">Make this job visible immediately</span>
            </div>
          </div>
          ...
          <div className="flex justify-end pt-4 border-t border-white/10">
            ...
            >
              {isSubmitting ? "Publishing..." : <><Plus size={16} /> Publish Job Opening</>}
            </button>
          </div>
        </form>
      </motion.div>
