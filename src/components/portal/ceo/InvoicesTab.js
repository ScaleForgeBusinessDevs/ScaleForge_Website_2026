...
function exportInvoicesCSV(data, convert, globalCurrency) {
  const headers = [
    "Invoice #",
    "Client",
    "Email",
  ...
  const rows = data.map((inv) => [
    ...
    inv.status,
    inv.total,
    convert(inv.total, inv.currency),
  ]);
...
export default function CeoInvoicesTab({
  addToast,
  globalCurrency = "USD",
  rates = {},
}) {
  ...
  const convert = (amount, fromCurr = "USD") => {
  ...
  const loadHistory = useCallback(async (invoiceId) => {
  ...
  const totalPaid = invoices
    ...
    .reduce((s, i) => s + convert(i.total, i.currency), 0);
  ...
  const totalInvoiced = invoices.reduce(
    (s, i) => s + convert(i.total, i.currency),
    0,
  );
  ...
  const handleMarkPaid = async (inv) => {
    ...
    if (txnError) {
      ...
      addToast(
        "error",
        "Invoice marked as paid, but failed to record transaction.",
      );
    }
    ...
    load();
    if (detail?.id === inv.id)
      setDetail({ ...inv, status: "paid", paid_at: new Date().toISOString() });
  };
  ...
  const calcLineTotal = (li) =>
    (parseFloat(li.quantity) || 0) * (parseFloat(li.unit_price) || 0);

  const calcSubtotal = () =>
    lineItems.reduce((s, li) => s + calcLineTotal(li), 0);

  const handleCreate = async () => {
    if (!form.client_name) {
      addToast("error", "Client name is required.");
      return;
    }
    if (lineItems.some((li) => !li.description || !li.unit_price)) {
      addToast("error", "All line items need a description and price.");
      return;
    }
        ...
        <div className="rounded-2xl border border-white/10 bg-white/3 p-8 space-y-8">
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-1">
                Total Amount ({detail.currency})
              </p>
              <p className="text-3xl font-bold text-white">
                {fmtMoney(detail.total, detail.currency)}
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-4 border-t border-white/8">
            {[
              ...
              {
                label: "Issued",
                value: new Date(detail.issued_date).toLocaleDateString(
                  "en-US",
                  { dateStyle: "medium" },
                ),
              },
          ...
          {/* Line Items */}
          <div>
            <p className="text-white/30 text-[9px] font-bold uppercase tracking-widest mb-3">
              Line Items
            </p>
            <div className="rounded-xl border border-white/8 overflow-hidden">
              ...
              {invItems.length === 0 ? (
                ...
                invItems.map((item) => (
                  <div
                    key={item.id}
                    className="grid grid-cols-[1fr_auto_auto_auto] gap-4 px-5 py-3 border-t border-white/5 text-sm"
                  >
                    <span className="text-white/70">{item.description}</span>
                    <span className="text-white/40">{item.quantity}</span>
                    <span className="text-white/40">
                      {fmtMoney(item.unit_price, detail.currency)}
                    </span>
                    <span className="text-white text-right font-semibold">
                      {fmtMoney(item.total, detail.currency)}
                    </span>
                  </div>
                ))
              )}
          ...
          <div className="flex flex-col items-end gap-2 pt-4 border-t border-white/8">
            <div className="flex gap-8 text-sm">
              ...
              <span className="text-white font-medium">
                {fmtMoney(detail.subtotal, detail.currency)}
              </span>
            </div>
            <div className="flex gap-8 text-sm">
              <span className="text-white/40">Tax ({detail.tax_rate}%)</span>
              <span className="text-white font-medium">
                {fmtMoney(detail.tax_amount, detail.currency)}
              </span>
            </div>
            <div className="flex gap-8 text-base font-bold border-t border-white/10 pt-2 mt-1">
          ...
          {detail.paid_at && (
            <div className="rounded-xl border border-green-500/20 bg-green-500/5 px-4 py-3 text-green-400 text-sm">
              ✓ Paid on{" "}
              {new Date(detail.paid_at).toLocaleDateString("en-US", {
                dateStyle: "long",
              })}
            </div>
          )}
          ...
          <div className="space-y-3">
            <p className="text-white/30 text-[9px] font-bold uppercase tracking-widest px-1">
              Payment History
            </p>
            {loadingHistory ? (
              ...
              <div className="rounded-xl border border-dashed border-white/10 p-4 text-center">
                <p className="text-white/20 text-xs italic text-center">
              ...
              <div className="space-y-2">
                {paymentHistory.map((txn) => (
                  <div
                    key={txn.id}
                    className="flex items-center justify-between px-4 py-3 rounded-xl bg-white/3 border border-white/5 group hover:bg-white/5 transition-colors"
                  >
                    ...
                    <div className="text-right">
                      <p className="text-green-400 text-xs font-bold font-mono">
                        +{fmtMoney(convert(txn.amount, "USD"))}
                      </p>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-white/60 text-xs">
                  {detail.pdf_url
                    ? "PDF document is attached to this invoice."
                    : "No PDF attached."}
                </p>
              </div>
              <div className="flex items-center gap-3">
                {detail.pdf_url && (
                  <button
                    onClick={() => handleViewPdf(detail.pdf_url)}
                ...
                <div className="relative">
                  ...
                  <button
                    disabled={uploadingPdf}
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-white/70 text-[10px] font-bold hover:bg-white/10 transition-all disabled:opacity-50"
                  >
                    {uploadingPdf ? (
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-5">
          <p className="text-black/40 text-[10px] font-bold uppercase tracking-widest mb-1">
            Collected
          </p>
          <p className="text-2xl font-bold text-green-400">
            {fmtMoney(totalPaid)}
          </p>
          <p className="text-black/30 text-[9px] mt-1">
        ...
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-5">
          ...
          <p className="text-black/30 text-[9px] mt-1">
            {invoices.filter((i) => i.status === "unpaid").length} open
          </p>
        </div>
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
          ...
          <p className="text-2xl font-bold text-red-400">
            {fmtMoney(totalOverdue)}
          </p>
          <p className="text-black/30 text-[9px] mt-1">
            {invoices.filter((i) => i.status === "overdue").length} critical
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <button
          onClick={() => exportInvoicesCSV(filtered, convert, globalCurrency)}
            ...
            >
              ...
              <div className="flex items-center gap-6">
                <div className="text-right hidden sm:block">
                  <p className="text-black/30 text-[9px] uppercase tracking-wider">
                    Issued
                  </p>
                ...
                <p className="text-black font-bold text-sm min-w-[80px] text-right">
                  {fmtMoney(convert(inv.total, inv.currency), globalCurrency)}
                </p>
                <ChevronRight
                  size={14}
                  className="text-black/20 group-hover:text-white/50 transition-colors"
                />
              </div>
            </motion.div>
          ))
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div
                      key={key}
                      className={key === "client_name" ? "col-span-2" : ""}
                    >
                  ...
                  <div>
                    ...
                    >
                      {["unpaid", "paid", "draft", "overdue", "cancelled"].map(
                        (s) => (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <label className="text-white/30 text-[9px] font-bold uppercase tracking-widest">
                      Line Items
                    </label>
                    <button
                      onClick={() =>
                      ...
                      className="text-yellow-400 text-[10px] font-bold hover:text-yellow-300 flex items-center gap-1"
                    >
                    ...
                    {lineItems.map((li, idx) => (
                      <div key={idx} className="flex gap-2 items-center">
                        ...
                        <span className="text-white/40 text-xs w-16 text-right">
                          {fmtMoney(calcLineTotal(li), form.currency)}
                        </span>
                        {lineItems.length > 1 && (
                          <button
                            onClick={() =>
                  ...
                  <div className="flex justify-end mt-3 text-sm gap-3">
                    ...
                    <span className="text-white font-bold">
                      {fmtMoney(calcSubtotal(), form.currency)}
                    </span>
                  </div>
                </div>
                <div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <div className="flex items-center justify-between w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm">
                      <span
                        className={
                          form.pdfFile ? "text-white" : "text-white/40"
                        }
                      >
                        {form.pdfFile
                          ? form.pdfFile.name
