        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div>
              ...
              <p className="text-4xl font-bold tracking-tight mt-2">{fmtMoney(totalBalance)}</p>
            </div>
          ...
          {/* Three Light Cards */}
          {[
            { label: "Net Cash Flow", value: netCashFlow,  icon: Activity,   color: netCashFlow >= 0 ? "text-green-600" : "text-red-600", trend: netCashFlow >= 0 ? "+5.1%" : "-2.1%", tColor: netCashFlow >= 0 ? "text-green-600 bg-green-50" : "text-red-600 bg-red-50", tIcon: netCashFlow >= 0 ? TrendingUp : TrendingDown },
            { label: "Total Savings", value: totalSavings, icon: PiggyBank,  color: "text-gray-900", trend: "+3.2%", tColor: "text-green-600 bg-green-50", tIcon: TrendingUp },
            { label: "Total Invested",value: totalInvested,icon: BarChart3,  color: "text-gray-900", trend: "+8.4%", tColor: "text-green-600 bg-green-50", tIcon: TrendingUp },
          ].map(({ label, value, icon: Icon, color, trend, tColor, tIcon: TIcon }, idx) => (
            <motion.div key={label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * (idx + 1) }}
              ...
              <div>
                <p className="text-gray-500 text-sm font-medium flex items-center gap-2 mb-2">
                  <span className="w-6 h-6 rounded-full bg-gray-50 flex items-center justify-center"><Icon size={12} className="text-gray-400" /></span>
                  {label}
                </p>
                <p className={`text-2xl font-bold tracking-tight ${color}`}>{fmtMoney(value)}</p>
              </div>
              <div className="mt-4 flex items-center gap-2">
                <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-bold ${tColor}`}>
                  <TIcon size={12} /> {trend}
                </span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-[24px] bg-white border border-gray-100 p-8 shadow-sm flex flex-col">
            <div className="flex items-center justify-between mb-8">
              <div>
                <h3 className="font-bold text-gray-900 text-lg">Spending Overview</h3>
                <p className="text-gray-400 text-sm">Income vs Expenses tracking</p>
              </div>
                  ...
                  <div key={i} className="flex flex-col items-center gap-4 flex-1 group">
                    <div className="flex items-end justify-center gap-2 w-full h-full relative">
                      <div className="absolute -top-12 opacity-0 group-hover:opacity-100 transition-opacity bg-gray-900 text-white px-3 py-1.5 rounded-lg text-xs font-mono flex flex-col items-center pointer-events-none z-10 shadow-xl">
                        <span className="text-green-400 font-bold">+{d.in}</span>
                    ...
                    <span className="text-gray-400 text-xs font-semibold">{d.day}</span>
                  </div>
                );
              })}
            </div>
          ...
          <div className="lg:col-span-1 rounded-[24px] bg-white border border-gray-100 p-6 shadow-sm flex flex-col h-[400px]">
            ...
            <div className="space-y-3 overflow-y-auto pr-2 custom-scrollbar flex-1">
              {accounts.map(acc => {
                const Icon = ACCOUNT_ICON[acc.type] ?? Wallet;
                return (
                  <div key={acc.id} className="group relative rounded-2xl border border-gray-100 bg-gray-50/50 p-4 hover:bg-white hover:border-gray-200 hover:shadow-sm transition-all">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold text-gray-900 truncate leading-tight">{acc.name}</p>
                        <p className="text-gray-400 text-xs truncate capitalize">{acc.type} • {acc.currency}</p>
                      </div>
                      <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 bg-white border border-gray-100 rounded-lg p-1 shadow-sm">
                        <button onClick={() => openEdit(acc)} className="p-1.5 hover:text-blue-600 text-gray-400"><Pencil size={14} /></button>
                        <button onClick={() => handleDeleteAccount(acc.id)} className="p-1.5 hover:text-red-600 text-gray-400"><Trash2 size={14} /></button>
                      </div>
                    </div>
                    <div className="flex items-end justify-between">
                      <div>
                        <p className="text-gray-400 text-[10px] uppercase tracking-widest font-bold mb-0.5">Balance</p>
                        <p className="font-bold font-mono text-base text-gray-900">{fmtMoney(convertAmount(acc.balance, acc.currency, globalCurrency))}</p>
                      </div>
                      {acc.currency !== globalCurrency && (
                        <p className="text-gray-400 text-xs font-mono bg-white px-2 py-0.5 rounded border border-gray-100">{fmtMoney(acc.balance, acc.currency)}</p>
                      )}
                    </div>
                  </div>
                );
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 rounded-[24px] bg-white border border-gray-100 p-6 shadow-sm flex flex-col">
            <div className="mb-6 flex items-center justify-between">
              ...
              <button className="text-sm font-semibold text-gray-400 hover:text-gray-900 transition-colors">View All</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 flex-1">
              {plans.length === 0 ? (
                <p className="text-gray-400 text-sm col-span-full text-center py-8">No active payment plans.</p>
              ) : plans.map(p => {
                ...
                return (
                  <div key={p.id} className="p-5 rounded-2xl border border-gray-100 bg-gray-50/50 hover:bg-white hover:border-gray-200 hover:shadow-md transition-all flex flex-col justify-between">
                    <div>
                      ...
                      <p className="text-gray-900 text-base font-bold mb-1">{p.client_name}</p>
                      <p className="text-gray-400 text-xs">{p.project_name}</p>
                    </div>
                    <div className="mt-6">
                      <div className="flex items-center justify-between mb-2">
                        <p className="text-gray-900 font-bold font-mono">{fmtMoney(remPkr)}</p>
                        <span className="text-[10px] font-bold text-gray-400 uppercase">Remaining</span>
                      </div>
                      <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden mb-2">
                        <motion.div initial={{ width: 0 }} animate={{ width: `${pct}%` }} className="h-full bg-gray-900 rounded-full" />
                      </div>
          ...
          <div className="lg:col-span-1 rounded-[24px] bg-white border border-gray-100 p-6 shadow-sm flex flex-col">
            ...
            <div className="flex-1 space-y-4">
              {recentTxns.length === 0 ? (
                ...
                const amt = convertAmount(t.amount, "USD", globalCurrency);
                return (
                  <div key={t.id} className="flex items-center justify-between group py-2 border-b border-gray-50 last:border-0">
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shrink-0 shadow-sm border border-gray-100 ${t.type === "income" ? "bg-green-50 text-green-600" : "bg-white text-gray-700"}`}>
                        {t.type === "income" ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
                      </div>
                      <div>
                        <p className="text-gray-900 text-sm font-bold">{t.description}</p>
                        <p className="text-gray-400 text-xs mt-0.5">{new Date(t.date).toLocaleDateString("en-US", { month: "short", day: "numeric" })} · {t.category}</p>
                      </div>
                    </div>
                    <p className={`text-base font-bold font-mono ${t.type === "income" ? "text-green-600" : "text-gray-900"}`}>
        ...
        {showAddAccount && (
          <div className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="flex items-center justify-between mb-8">
                <h3 className="font-bold text-gray-900 text-xl">{editingAccount ? "Edit Account" : "Add Account"}</h3>
                <button onClick={() => { setShowAddAccount(false); resetForm(); setEditingAccount(null); }} className="text-gray-400 hover:text-gray-900 bg-gray-50 hover:bg-gray-100 rounded-full p-2 transition-colors"><X size={18} /></button>
              </div>
              <div className="space-y-5">
                {([
                  ...
                  <div key={key}>
                    <label className="text-gray-500 text-[10px] font-bold uppercase tracking-widest block mb-2">{label}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-gray-500 text-[10px] font-bold uppercase tracking-widest block mb-2">Type</label>
                    <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value as Account["type"] }))}
                      ...
                      {["checking","savings","investment","wallet"].map(t => <option key={t} value={t}>{t.charAt(0).toUpperCase() + t.slice(1)}</option>)}
                    </select>
                  </div>
                  <div>
                    <label className="text-gray-500 text-[10px] font-bold uppercase tracking-widest block mb-2">Currency</label>
                    <select value={form.currency} onChange={e => setForm(f => ({ ...f, currency: e.target.value }))}
                      className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm text-gray-900 outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20 transition-all appearance-none">
                      {["USD","PKR","EUR","GBP"].map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>
                </div>
                ...
                <button onClick={handleSaveAccount}
                  ...
                  <Check size={18} /> {editingAccount ? "Save Changes" : "Add Account"}
                </button>
              </div>
            </motion.div>
          </div>
