...
export default function CeoSalariesTab({
  addToast,
  globalCurrency = "USD",
  rates = {},
}) {
  // Bridge conversion: stored_currency -> PKR -> globalCurrency
  const convertSalary = (amount, storedCurr) => {
    const fromRate =
      storedCurr === "PKR" ? 1 : (rates[storedCurr] ?? rates["USD"] ?? 278.5);
  ...
  const [showSalesForm, setShowSalesForm] = useState(null); // employee_id
  ...
  const totalMonthlyBase = activeSalaries.reduce(
    (acc, s) => acc + convertSalary(s.base_salary, s.currency),
    0,
  );
  ...
  const handleSave = async () => {
    if (!form.employee_name || !form.position || !form.base_salary) {
      addToast("error", "Name, position, and base salary are required.");
      return;
    }
    const payload = {
  ...
  const handleSaveSales = async (empId) => {
    if (!salesForm.sales_amount) {
      addToast("error", "Sales amount required.");
      return;
    }
    const emp = salaries.find((s) => s.id === empId);
    if (!emp) return;
    const salesAmt = parseFloat(salesForm.sales_amount);
    const commRate = salesForm.commission_rate
      ? parseFloat(salesForm.commission_rate)
      : emp.commission_rate;
    const payload = {
      employee_id: empId,
      month: salesForm.month,
      year: salesForm.year,
    ...
    if (error) {
      addToast("error", "Failed to save sales record.");
      return;
    }
    addToast("success", "Sales record saved.");
    setShowSalesForm(null);
  ...
  const deleteSalesRecord = async (id) => {
    const { error } = await supabase
      ...
      .eq("id", id);
    if (error) {
      addToast("error", "Failed to delete sales record.");
      return;
    }
    addToast("success", "Sales record deleted.");
    load();
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="rounded-2xl border border-white/6 bg-white/3 p-5">
          ...
          <p className="text-2xl font-bold text-white">
            {fmtMoney(totalMonthlyBase)}
          </p>
          <p className="text-white/30 text-xs mt-1">
            {activeSalaries.length} active employees
          </p>
        </div>
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-5">
          <p className="text-emerald-400/50 text-[10px] font-bold uppercase tracking-widest mb-1">
            Current Month Commissions
          </p>
          <p className="text-2xl font-bold text-emerald-400">
            {fmtMoney(currentMonthCommissions)}
          </p>
        ...
        <div className="rounded-2xl border border-purple-500/20 bg-purple-500/5 p-5">
          <p className="text-purple-400/50 text-[10px] font-bold uppercase tracking-widest mb-1">
            Senior Leadership
          </p>
          <p className="text-2xl font-bold text-purple-400">
            {salaries.filter((s) => s.role_type === "senior").length}
          </p>
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-white/50 text-sm font-bold uppercase tracking-widest">
          Employee Salaries & Roles
        </h2>
        <button
          onClick={() => {
            setEditing(null);
      ...
      <div className="space-y-2">
        {salaries.length === 0 ? (
          ...
          salaries.map((emp) => {
            ...
            const empSales = salesRecords.filter(
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${cfg.color.split(" ")[1]}`}
                  >
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-white/80 text-sm font-bold font-mono">
                      {fmtMoney(convertSalary(emp.base_salary, emp.currency))}
                    </p>
                      ...
                      <div className="px-5 py-4 space-y-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          <div>
                            ...
                            <span
                              className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-bold uppercase tracking-wider ${cfg.color}`}
                            >
                          ...
                          {emp.role_type === "sales" && (
                            <div>
                              <p className="text-emerald-400/50 text-[9px] font-bold uppercase tracking-widest mb-1">
                                Commission Rate
                              </p>
                          ...
                          {emp.role_type === "senior" && (
                            <div className="col-span-2 md:col-span-1">
                              <p className="text-purple-400/50 text-[9px] font-bold uppercase tracking-widest mb-1">
                                Impact / Ownership
                              </p>
                              <p className="text-white/70 text-sm italic">
                                {emp.impact_notes ||
                                  "No impact notes specified."}
                              </p>
                            </div>
                          )}
                        </div>
                          ...
                          <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                            ...
                            {showSalesForm === emp.id && (
                              <div className="mb-4 bg-white/5 p-3 rounded-lg border border-white/10 grid grid-cols-2 md:grid-cols-5 gap-2 items-end">
                                <div>
                                  <label className="text-white/30 text-[9px] font-bold uppercase block mb-1">
                                    Month
                                  </label>
                                  <input
                                    type="number"
                                    ...
                                    className="w-full bg-black/50 border border-white/10 rounded px-2 py-1.5 text-white text-xs outline-none focus:border-emerald-500/50"
                                  />
                                </div>
                                <div>
                                  <label className="text-white/30 text-[9px] font-bold uppercase block mb-1">
                                ...
                                <div>
                                  ...
                                  <input
                                    ...
                                    value={salesForm.commission_rate}
                                    onChange={(e) =>
                                      setSalesForm((f) => ({
                                        ...f,
                                ...
                                <div className="flex gap-1">
                                  <button
                                    onClick={() => handleSaveSales(emp.id)}
                                    className="flex-1 bg-emerald-500/20 text-emerald-400 rounded py-1.5 flex justify-center hover:bg-emerald-500/30 transition-colors"
                                  >
                                    <Check size={14} />
                                  </button>
                                  <button
                                    onClick={() => setShowSalesForm(null)}
                                    className="flex-1 bg-white/5 text-white/40 rounded py-1.5 flex justify-center hover:bg-white/10 transition-colors"
                                  >
                                    <X size={14} />
                                  </button>
                                </div>
                            ...
                            {empSales.length === 0 ? (
                              ...
                              <div className="space-y-1">
                                {empSales.map((r) => (
                                  <div
                                    key={r.id}
                                    className="flex items-center justify-between text-xs py-1.5 border-b border-white/5 last:border-0 group"
                                  >
                                    <span className="text-white/50 w-24">
                                      {r.month}/{r.year}
                                    </span>
                                    <span className="text-white/70 font-mono w-24">
                                      {fmtMoney(
                                        convertSalary(r.sales_amount, "USD"),
                                      )}{" "}
                                      sales
                                    </span>
                                    <span className="text-emerald-400 font-bold font-mono w-24">
                                      +
                                      {fmtMoney(
                                        convertSalary(
                                          r.commission_earned,
                                    ...
                                    <span className="text-white/30 text-[9px] italic flex-1 truncate">
                                      {r.notes}
                                    </span>
                                    <button
                                      onClick={() => deleteSalesRecord(r.id)}
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    <label className="text-white/30 text-[9px] font-bold uppercase tracking-widest block mb-1.5">
                      Employee Name *
                    </label>
                    <input
                      type="text"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm outline-none focus:border-yellow-500/50"
                    >
                      <option value="regular" className="bg-[#0d0d0d]">
                        Regular
                      </option>
                      <option value="sales" className="bg-[#0d0d0d]">
                        Sales
                      </option>
                      <option value="senior" className="bg-[#0d0d0d]">
                        Senior Leadership
                      </option>
                      <option value="contractor" className="bg-[#0d0d0d]">
                        Contractor
                      </option>
                    </select>
                  </div>
                  <div>
                    ...
                    >
                      ...
                      <option value="on_leave" className="bg-[#0d0d0d]">
                        On Leave
                      </option>
                    </select>
                  </div>
                  ...
                  {form.role_type === "sales" && (
                    <div>
                      <label className="text-emerald-400/50 text-[9px] font-bold uppercase tracking-widest block mb-1.5">
                        Commission Rate %
                      </label>
                      <input
                        type="number"
                  ...
                  {form.role_type === "senior" && (
                    <div className="col-span-2">
                      <label className="text-purple-400/50 text-[9px] font-bold uppercase tracking-widest block mb-1.5">
                        Impact / Ownership Notes
                      </label>
                      <textarea
                        ...
                        onChange={(e) =>
                          setForm((f) => ({
                            ...f,
                            impact_notes: e.target.value,
                          }))
                        }
                        rows={2}
                        placeholder="What does this senior leader own?"
