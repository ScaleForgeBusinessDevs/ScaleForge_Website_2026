    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-5">
          <p className="text-black/40 text-[10px] font-bold uppercase tracking-widest mb-1">Collected</p>
          <p className="text-2xl font-bold text-green-400">{fmtMoney(totalPaid)}</p>
          <p className="text-black/30 text-[9px] mt-1">{invoices.filter(i => i.status === "paid").length} paid</p>
        </div>
        <div className="rounded-2xl border border-yellow-500/20 bg-yellow-500/5 p-5">
          <p className="text-black/40 text-[10px] font-bold uppercase tracking-widest mb-1">Pending</p>
          <p className="text-2xl font-bold text-yellow-400">{fmtMoney(totalPending)}</p>
          <p className="text-black/30 text-[9px] mt-1">{invoices.filter(i => i.status === "unpaid").length} open</p>
        </div>
        <div className="rounded-2xl border border-red-500/20 bg-red-500/5 p-5">
          <p className="text-black/40 text-[10px] font-bold uppercase tracking-widest mb-1">Overdue</p>
          <p className="text-2xl font-bold text-red-400">{fmtMoney(totalOverdue)}</p>
          <p className="text-black/30 text-[9px] mt-1">{invoices.filter(i => i.status === "overdue").length} critical</p>
        </div>
      </div>
        ...
        {filtered.length === 0 ? (
          ...
          <motion.div key={inv.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-black font-semibold text-sm">{inv.invoice_number}</p>
                <p className="text-black/40 text-xs">{inv.client_name} {inv.client_email ? `· ${inv.client_email}` : ""}</p>
              </div>
            </div>
            <div className="flex items-center gap-6">
              <div className="text-right hidden sm:block">
                <p className="text-black/30 text-[9px] uppercase tracking-wider">Issued</p>
                <p className="text-black/50 text-xs">{new Date(inv.issued_date).toLocaleDateString("en-US", { dateStyle: "medium" })}</p>
              </div>
              <div className="text-right hidden md:block">
                <p className="text-black/30 text-[9px] uppercase tracking-wider">Due</p>
                <p className="text-black/50 text-xs">{inv.due_date ? new Date(inv.due_date).toLocaleDateString("en-US", { dateStyle: "medium" }) : "—"}</p>
              </div>
              <StatusBadge status={inv.status} />
              <p className="text-black font-bold text-sm min-w-[80px] text-right">{fmtMoney(convert(inv.total, inv.currency), globalCurrency)}</p>
              <ChevronRight size={14} className="text-black/20 group-hover:text-white/50 transition-colors" />
            </div>
          </motion.div>
        ))}
      </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={key} className={key === "client_name" ? "col-span-2" : ""}>
                      ...
                      <input type={type} placeholder={placeholder} value={(form[key as keyof typeof form] as string | undefined) ?? ""}
