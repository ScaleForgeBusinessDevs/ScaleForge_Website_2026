...
export default function CeoTransactionsTab({
  addToast,
  globalCurrency = "USD",
  rates = {},
}) {
  ...
  const filtered = txns.filter((t) => {
    ...
    if (
      filterClient &&
      !t.client_name?.toLowerCase().includes(filterClient.toLowerCase())
    )
      return false;
  ...
  const handleAdd = async () => {
    if (!form.description || !form.amount) {
      addToast("error", "Description and amount are required.");
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            color:
              totalIncome >= totalExpense ? "text-green-400" : "text-red-400",
      ...
      <div className="rounded-2xl border border-white/8 overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto_auto_auto] gap-0 bg-white/3 px-5 py-3 text-[9px] font-bold uppercase tracking-widest text-white/30 border-b border-white/8">
          ...
          <span className="text-right pr-4">Client</span>
          ...
          filtered.map((t, i) => (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${t.type === "income" ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}
                >
                  {t.type === "income" ? (
                    <TrendingUp size={12} />
                  ) : (
                ...
                <div className="min-w-0">
                  ...
                  {t.notes && (
                    <p className="text-white/30 text-xs truncate">{t.notes}</p>
                  )}
                </div>
              </div>
              <span className="text-white/40 text-xs pr-4 text-right">
                {t.category}
              </span>
              <span className="text-white/40 text-xs pr-4 text-right">
                {t.client_name ?? "—"}
              </span>
              <span className="text-white/30 text-xs pr-4 text-right font-mono">
              ...
              <div className="flex items-center gap-2 justify-end">
                <span
                  className={`text-sm font-bold font-mono ${t.type === "income" ? "text-green-400" : "text-red-400"}`}
                >
                  {t.type === "income" ? "+" : "-"}
                  {fmtMoney(convert(t.amount))}
                </span>
                <button
                ...
                {[
                  {
                    ...
                    key: "description",
                    type: "text",
                    placeholder: "e.g. Project payment from client",
                  },
                  {
                    label: "Amount",
                    key: "amount",
