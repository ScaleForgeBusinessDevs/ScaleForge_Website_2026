...
export default function CeoPaymentPlansTab({
  addToast,
  globalCurrency = "USD",
  rates = {},
}) {
  // Bridge conversion: stored_currency -> PKR -> globalCurrency
  const convertPlan = (amount, storedCurr) => {
    const fromRate =
  ...
  const totalOutstanding = plans
    ...
    .reduce(
      (s, p) => s + convertPlan(p.remaining_balance, p.currency || "USD"),
      0,
    );
    ...
    (s, p) =>
      ...
      convertPlan(
        (p.upfront_paid ? p.upfront_amount : 0) +
          p.installments_paid * p.installment_amount,
        p.currency || "USD",
      ),
    0,
  );
  ...
  const recordPayment = async (p) => {
    if (p.installments_paid >= p.total_installments) {
      addToast("info", "All installments already paid.");
      return;
    }
    const newPaid = p.installments_paid + 1;
    const newRemaining = Math.max(
      0,
      p.remaining_balance - p.installment_amount,
    );
    ...
    if (txnError) {
      ...
      addToast(
        "error",
        "Payment recorded in plan, but failed to record transaction.",
      );
    }
    ...
    addToast(
      "success",
      newStatus === "completed"
        ? `✓ Plan for ${p.client_name} fully paid!`
        : `Payment #${newPaid} recorded for ${p.client_name}.`,
    );
    load();
  };

  const recordUpfrontPayment = async (p) => {
    if (p.upfront_paid) {
      addToast("info", "Upfront payment already recorded.");
      return;
    }
    ...
    if (error) {
      ...
      addToast(
        "error",
        `Failed to mark upfront as paid: ${error.message || "Unknown error"}`,
      );
      return;
    }
    ...
    if (txnError) {
      ...
      addToast(
        "error",
        `Upfront marked as paid, but transaction failed: ${txnError.message}`,
      );
    } else {
        ...
        {[
          {
            ...
            sub: `${activePlans} active plan${activePlans !== 1 ? "s" : ""}`,
          ...
          <div
            key={c.label}
            className="rounded-xl border border-border bg-card px-5 py-4 shadow-sm transition-all"
          >
            <p className="opacity-40 text-[9px] font-bold uppercase tracking-widest mb-1">
              {c.label}
            </p>
      ...
      <div className="space-y-2">
        {filtered.length === 0 ? (
          ...
          filtered.map((p) => {
            ...
            const pct =
              p.total_installments > 0
                ? Math.round((p.installments_paid / p.total_installments) * 100)
                : 0;
            ...
            return (
              ...
              >
                {/* Row */}
                <div
                  className="flex items-center gap-4 px-5 py-4 hover:bg-foreground/[0.02] transition-colors cursor-pointer"
                  onClick={() => setExpanded(isExpanded ? null : p.id)}
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="opacity-90 text-sm font-medium truncate">
                        {p.client_name}
                      </p>
                      <span
                        className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[9px] font-bold uppercase tracking-wider ${cfg.color}`}
                      >
                        <Icon size={8} /> {cfg.label}
                      </span>
                    </div>
                    <p className="opacity-30 text-xs truncate">
                      {p.project_name}
                    </p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="opacity-90 text-sm font-bold font-mono">
                      {fmtMoney(
                        convertPlan(p.remaining_balance, p.currency || "USD"),
                      )}
                    </p>
                  ...
                  <div className="text-right shrink-0 hidden sm:block">
                    <p className="opacity-60 text-xs font-mono">
                      {fmtMoney(
                        convertPlan(p.installment_amount, p.currency || "USD"),
                      )}
                    </p>
                    <p className="opacity-30 text-[9px]">
                      ...
                      <div className="px-5 py-4 space-y-4">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                          {[
                            ...
                            {
                              label: "Upfront Status",
                              value: p.upfront_paid ? "Paid" : "Pending",
                              color: p.upfront_paid
                                ? "text-emerald-400"
                                : "text-yellow-400",
                            },
                            {
                              label: "Paid Installments",
                              value: `${p.installments_paid} / ${p.total_installments}`,
                            },
                          ].map((f) => (
                            <div
                              key={f.label}
                              className="bg-foreground/[0.03] rounded-lg px-3.5 py-2.5"
                            >
                              <p className="opacity-30 text-[9px] font-bold uppercase tracking-wider mb-0.5">
                                {f.label}
                              </p>
                              <p
                                className={`text-xs font-semibold ${f.color || "opacity-70"}`}
                              >
                                {f.value}
                              </p>
                            </div>
                          ...
                          {loadingHistory ? (
                            ...
                            <div className="rounded-xl border border-dashed border-white/8 p-4 text-center">
                              <p className="text-white/20 text-[10px] italic">
                                No transactions linked to this plan.
                              </p>
                            </div>
                          ) : (
                            <div className="space-y-2">
                              {paymentHistory.map((txn) => (
                                ...
                                >
                                  ...
                                  <div className="text-right">
                                    <p className="text-emerald-500/80 text-xs font-bold font-mono">
                                      +
                                      {fmtMoney(convertPlan(txn.amount, "USD"))}
                                    </p>
                        ...
                        {p.notes && (
                          <p className="opacity-40 text-xs italic">{p.notes}</p>
                        )}
                        ...
                        <div className="flex flex-wrap gap-2 pt-1">
                          ...
                          {!p.upfront_paid && p.upfront_amount > 0 && (
                            <button
                              onClick={() => recordUpfrontPayment(p)}
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <h3 className="text-white font-semibold text-base">
                  {editing ? "Edit Payment Plan" : "New Payment Plan"}
                </h3>
                <button
                  onClick={() => {
                    setShowForm(false);
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  {
                    label: "Already Paid (count)",
                    key: "installments_paid",
                    type: "number",
                  },
                ...
                <div>
                  ...
                  >
                    <option value="monthly" className="bg-[#0d0d0d]">
                      Monthly
                    </option>
                    <option value="bimonthly" className="bg-[#0d0d0d]">
                      Bi-Monthly
                    </option>
                    <option value="quarterly" className="bg-[#0d0d0d]">
                      Quarterly
                    </option>
                  </select>
                </div>
                ...
                <div className="col-span-2 flex items-center gap-3 py-1">
                  <input
                    ...
                    onChange={(e) =>
                      setForm((f) => ({ ...f, upfront_paid: e.target.checked }))
                    }
                    className="w-4 h-4 rounded border-white/10 bg-white/5 accent-yellow-500"
                  />
                  <label
                    htmlFor="upfront_paid"
                    className="text-white/60 text-xs font-medium cursor-pointer select-none"
                  >
