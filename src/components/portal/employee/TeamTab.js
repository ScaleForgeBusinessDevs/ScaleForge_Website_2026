...
export default function TeamTab({ profileId }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const ids = (memberLinks ?? []).map((m) => m.profile_id);
      ...
      if (teamData?.lead_id) {
        const leadProfile = (profiles ?? []).find(
          (p) => p.id === teamData.lead_id,
        );
      ...
      const { data: tasks } = await supabase
        .from("tasks")
        .select("*")
        .eq("assigned_team", membership.team_id)
        .neq("status", "done");
      setTeamTasks(tasks ?? []);
    })();
  }, [profileId]);
      ...
      <div>
        ...
        {teamTasks.length === 0 && (
          <div className="flex items-center gap-3 text-white/20 py-8">
            <CheckSquare size={16} />{" "}
            <span className="text-sm">No active team tasks.</span>
          </div>
        )}
