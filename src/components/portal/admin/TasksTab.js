  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <span
          className={`px-2 py-0.5 rounded-full text-[9px] font-bold uppercase border ${PRIORITY_STYLES[task.priority]}`}
        >
...
export default function TasksTab({ addToast }) {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: p } = await supabase
        ...
        .select("*")
        .eq("role", "employee");
      const empMap = Object.fromEntries((p ?? []).map((e) => [e.id, e]));
      setTasks(
        (t ?? []).map((task) => ({
          ...task,
          assignee: empMap[task.assigned_to ?? ""],
        })),
      );
      setEmployees(p ?? []);
    };
    ...
    <div className="space-y-6">
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-400 text-white text-xs font-bold uppercase tracking-wider transition-colors shadow-[0_0_16px_rgba(249,115,22,0.3)]"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {COLUMNS.map((col) => {
          ...
          return (
            <div
              key={col.id}
              className={`rounded-2xl border ${col.color} bg-white/3 p-4 space-y-3`}
            >
              <div className="flex items-center justify-between mb-2">
                <h3 className="text-white/60 text-[10px] font-bold uppercase tracking-widest">
                  {col.label}
                </h3>
                <span className="text-white/30 text-[10px] font-bold bg-white/5 px-2 py-0.5 rounded-full">
                  {colTasks.length}
                </span>
              </div>
              <AnimatePresence>
              ...
              {colTasks.length === 0 && (
                <p className="text-white/15 text-xs text-center py-6">Empty</p>
              )}
            </div>
          );
        })}
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleCreate} className="space-y-5">
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-orange-500 hover:bg-orange-400 text-white font-bold text-sm uppercase tracking-wider transition-colors mt-4"
                >
                  Create Task
                </button>
              </form>
