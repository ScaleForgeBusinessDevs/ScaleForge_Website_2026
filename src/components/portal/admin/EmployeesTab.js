...
function EmployeeCard({ emp, onUpdate, onDelete, onToggleRole }) {
  ...
  // Check if modified
  const isModified =
    expectedIn !== (emp.expected_time_in || "") ||
    expectedOut !== (emp.expected_time_out || "");
  ...
  const saveHours = async () => {
    ...
    onUpdate({
      ...emp,
      expected_time_in: expectedIn,
      expected_time_out: expectedOut,
    });
    setSaving(false);
  };
  ...
  return (
    ...
    >
      ...
      <div className="bg-black/20 p-3 rounded-xl border border-white/5 mb-4">
        <p className="text-white/40 text-[9px] font-bold uppercase tracking-widest mb-2">
          Expected Hours
        </p>
...
export default function EmployeesTab({ addToast }) {
