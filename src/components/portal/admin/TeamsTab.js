...
export default function TeamsTab({ addToast }) {
  ...
  const load = async () => {
    ...
    setTeams(
      (teamsData ?? []).map((t) => {
        ...
        return {
          ...t,
          members: memberIds.map((id) => profMap[id]).filter(Boolean),
  ...
  const handleAddMember = async (teamId, profileId) => {
    await supabase
      .from("team_members")
      .upsert({ team_id: teamId, profile_id: profileId });
    addToast("success", "Member added to team.");
    load();
  };

  const handleRemoveMember = async (teamId, profileId) => {
    await supabase
      .from("team_members")
      .delete()
    ...
    <div className="space-y-6">
      <div className="flex justify-end">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 rounded-xl bg-orange-500 hover:bg-orange-400 text-white text-xs font-bold uppercase tracking-wider transition-colors"
        >
            ...
            <div className="p-5">
              ...
              <div className="flex items-center gap-1 flex-wrap mb-3">
                {team.members.slice(0, 6).map((m) => (
                  <div
                    key={m.id}
                    className="w-7 h-7 rounded-full bg-gradient-to-br from-orange-500 to-pink-600 flex items-center justify-center text-white text-[10px] font-bold"
                    title={m.full_name}
                  >
              ...
              <button
                ...
                className="flex items-center gap-1.5 text-white/40 hover:text-white/70 text-[10px] font-bold uppercase tracking-wider transition-colors"
              >
                {team.expanded ? (
                  <>
                ...
                >
                  <div className="p-4 space-y-2">
                    {team.members.map((m) => (
                      <div
                        key={m.id}
                        className="flex items-center justify-between px-3 py-2 rounded-lg bg-white/3 hover:bg-white/6 transition-colors"
                      >
                        <div className="flex items-center gap-2">
                          <div className="w-6 h-6 rounded-full bg-gradient-to-br from-orange-500 to-pink-600 flex items-center justify-center text-white text-[9px] font-bold">
                            {m.full_name.charAt(0)}
                          </div>
                    ...
                    >
                      ...
                      {employees
                        .filter((e) => !team.members.find((m) => m.id === e.id))
                        .map((e) => (
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleCreateTeam} className="space-y-4">
                {[
                  { label: "Team Name", key: "name", required: true },
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-orange-500 hover:bg-orange-400 text-white font-bold text-sm uppercase tracking-wider transition-colors mt-2"
                >
                  Create Team
                </button>
              </form>
