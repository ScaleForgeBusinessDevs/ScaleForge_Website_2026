...
function StatCard({ label, value, icon: Icon, gradient, delay }) {
  return (
    ...
    >
      <div
        className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${gradient} blur-[60px]`}
      />
  ...
  return (
    ...
    >
      ...
      <div className="relative w-10 h-10 rounded-full bg-gradient-to-br from-orange-500 to-orange-700 flex items-center justify-center text-white font-bold text-sm shrink-0">
        ...
        <span
          className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-black ${STATUS_COLORS[emp.status] ?? "bg-white/20"} ${emp.status === "Working" ? "animate-pulse" : ""}`}
        />
      </div>
      ...
      <div className="relative">
        <button
          onClick={() => setOpen((v) => !v)}
          className="p-1.5 rounded-lg text-white/20 hover:text-white/60 hover:bg-white/8 transition-colors opacity-0 group-hover:opacity-100"
        >
        ...
        {open && (
          <div className="absolute right-0 top-8 z-30 bg-black border border-white/10 rounded-xl overflow-hidden shadow-2xl w-36">
            ...
            <button
              onClick={onRemove}
              className="flex items-center gap-2 px-4 py-2.5 text-xs text-red-400 hover:bg-red-500/10 w-full transition-colors"
            >
...
export default function OverviewTab({ addToast }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      const emps = (profiles ?? []).map((p) => ({
        ...p,
        status: statusMap[p.id] ?? "Offline",
      }));
  ...
  return (
    <div className="space-y-10">
      ...
      {/* Employee List */}
      <div>
        <h2 className="text-white/60 text-[10px] font-bold uppercase tracking-widest mb-4">
          Team Status
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {employees.length === 0 && (
            <p className="text-white/30 text-sm col-span-2 py-8 text-center">
              No employees yet — use Onboarding to add team members.
            </p>
          )}
          {employees.map((emp) => (
            <EmployeeCard
