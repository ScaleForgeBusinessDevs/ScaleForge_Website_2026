...
function aggregateDayLogs(logs) {
  const shifts = [];
  const breaks = [];
...
function groupLogs(logs) {
  ...
  logs.forEach((log) => {
    ...
    if (!grouped[log.date][log.profile_id])
      grouped[log.date][log.profile_id] = [];
    grouped[log.date][log.profile_id].push(log);
  });
  return grouped;
}

function AdminAttendanceCard({ emp, dayLogs, delayIndex, onMarkAbsent }) {
  const [expanded, setExpanded] = useState(false);
  const hours = calcHoursWorked(
    [...dayLogs].sort(
      (a, b) =>
  ...
  const isOngoing =
    agg.shifts.length > 0 && agg.shifts[agg.shifts.length - 1].out === null;
                    ...
                    <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                      <div className="flex items-center gap-2 text-green-400 mb-2">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-widest">
                          Time In {agg.shifts.length > 1 ? `#${i + 1}` : ""}
                        </span>
                      </div>
                      <p className="text-white font-medium text-lg">
                        {s.in ? formatTime(s.in) : "—"}
                      </p>
                    </div>
                    <div className="bg-black/20 rounded-xl p-4 border border-white/5">
                      <div className="flex items-center gap-2 text-red-400 mb-2">
                        <LogOut size={14} />
                        <span className="text-[10px] font-bold uppercase tracking-widest">
                          Time Out {agg.shifts.length > 1 ? `#${i + 1}` : ""}
                        </span>
                      </div>
                      <p className="text-white font-medium text-lg">
                        {s.out ? formatTime(s.out) : "—"}
                      </p>
                    </div>
                  </div>
                ))}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    {agg.breaks.map((b, i) => (
                      <div
                        key={i}
                        className="flex items-center justify-between text-sm text-white/70 bg-black/20 rounded-xl p-3 border border-white/5"
                      >
...
export default function AttendanceTab({ addToast }) {
  ...
  const filteredLogs = logs.filter((log) => {
    ...
    if (selectedTeam !== "all") {
      const isMember = teamMembers.some(
        (tm) => tm.team_id === selectedTeam && tm.profile_id === emp.id,
      );
            ...
            <button
              onClick={async () => {
                ...
                if (existing) {
                  addToast(
                    "info",
                    "Employee is already marked absent on this date.",
                  );
                  return;
                }
