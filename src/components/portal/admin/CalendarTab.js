...
export default function CalendarTab({ addToast, role = "admin" }) {
      ...
      <div className="rounded-2xl border border-white/8 bg-white/3 overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b border-white/8">
          {DAY_NAMES.map((d) => (
            <div
              key={d}
              className="py-3 text-center text-[10px] font-bold uppercase tracking-widest text-white/30"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7">
          {Array.from({ length: firstDayOfMonth }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="min-h-[90px] border-b border-r border-white/5 p-2"
            />
          ))}
          ...
          >
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-white font-bold text-lg">
                  {selectedMeeting ? "Meeting Details" : "Add Meeting"}
                </h2>
                <button
                  onClick={() => setShowModal(false)}
                  ...
                  <div className="flex flex-col gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-xs text-white/60 truncate">
                        {selectedMeeting.description?.match(
                          /https?:\/\/[^\s]+/,
                        )?.[0] || "No link provided"}
                      </div>
                      {selectedMeeting.description?.match(
                      ...
                      {selectedMeeting.attendees?.length > 0 ? (
                        selectedMeeting.attendees.map((id) => {
                          ...
                          return (
                            <div
                              key={id}
                              className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] text-white/60 flex items-center gap-2"
                            >
