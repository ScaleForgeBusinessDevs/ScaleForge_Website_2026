...
export default function PageHero({ eyebrow, title, subhead, primaryCta, secondaryCta, stats }: PageHeroProps) {
  return (
    <section className="relative flex min-h-screen items-center overflow-hidden bg-[#08090a]">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1440px] px-6 pb-20 pt-32 text-center lg:px-10 lg:pt-28">
        <Reveal className="mx-auto flex max-w-6xl flex-col items-center">
          ...
          {subhead && (
            <p className="mx-auto mt-5 max-w-2xl text-[15px] leading-relaxed text-white/45">{subhead}</p>
          )}
        ...
        {stats && stats.length > 0 && (
          <Reveal stagger className="mx-auto mt-16 grid max-w-3xl grid-cols-2 gap-x-6 gap-y-8 sm:grid-cols-4">
            {stats.map((stat) => (
              <div key={stat.label} className="border-t border-white/[0.07] pt-4 text-left">
                <p className="text-[26px] font-bold tracking-[-0.01em] text-white">{stat.value}</p>
                <p className="mt-1 text-[11px] uppercase leading-snug tracking-[0.08em] text-white/40">{stat.label}</p>
              </div>
            ))}
          </Reveal>
        )}
